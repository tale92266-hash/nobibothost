...
function resolveVariablesRecursively(text, senderName, receivedMessage, processingTime, groupName, isGroup, VARIABLES, messageHistory, regexMatch = null, matchedRuleId = null, totalMsgs = 0, messageStats = null, maxIterations = 10) {
    let result = text;
...
function pickRandomReply(replyText, senderName, msg, processingTime, groupName, isGroup, VARIABLES, messageHistory) {
    const replies = replyText.split('<#>').map(r => r.trim()).filter(Boolean);
    if (replies.length === 0) {
        return null;
    }
    const selectedReply = pick(replies);
    return resolveVariablesRecursively(selectedReply, senderName, msg, processingTime, groupName, isGroup, VARIABLES, messageHistory);
}
