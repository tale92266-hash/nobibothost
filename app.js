...
(async () => {
    ...
    const files = [
        ...
        {
            path: path.join(dataDir, "owner_rules.json"),
            content: {
                rules: []
            }
        },
        {
            path: path.join(dataDir, "owner_list.json"),
            content: []
        },
        {
            path: path.join(dataDir, "ignored_override_users.json"),
            content: []
        },
        {
            path: path.join(dataDir, "specific_override_users.json"),
            content: []
        },
        {
    ...
    await db.mongoose.connection.once('open', async () => {
        ...
        const settingsLoaded = await require('./db').loadSettingsFromFiles();
        if (!settingsLoaded) {
            console.log('⚠️ Settings files not found. Restoring from MongoDB...');
            await require('./db').restoreSettingsFromDb();
        }
