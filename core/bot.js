...
async function processMessage(msg, sessionId = "default", sender) {
    ...
    const cooldownKey = `${sessionId}-rule-cooldown`;
    ...
    if (getIsAutomationEnabled() && msg.startsWith('/') && automationRules.length > 0) {
        for (const rule of automationRules) {
            const ruleCooldownKey = `${sessionId}-${rule.RULE_NUMBER}`;
            if (ruleCooldowns.has(ruleCooldownKey) && Date.now() < ruleCooldowns.get(ruleCooldownKey)) {
            ...
            if (userCanRun && matchesTrigger(msg, rule.KEYWORDS, rule.RULE_TYPE)) {
                ...
                if (rule.COOLDOWN > 0) {
                    ...
                    ruleCooldowns.set(ruleCooldownKey, cooldownTime);
    ...
    if (!replies && !isOwner) {
        ...
        for (let rule of getRules()) {
            const ruleCooldownKey = `${sessionId}-${rule.RULE_NUMBER}`;
            if (ruleCooldowns.has(ruleCooldownKey) && Date.now() < ruleCooldowns.get(ruleCooldownKey)) {
                console.log(`🚫 Rule "${rule.RULE_NAME}" is on cooldown for this user.`);
                continue;
            }
    ...
    let finalReplies = null;
    
    if (replies) {
        if (cooldown > 0 && matchedRuleId) {
            const ruleCooldownKey = `${sessionId}-${matchedRuleId}`;
            const cooldownTime = Date.now() + (cooldown * 1000);
            ruleCooldowns.set(ruleCooldownKey, cooldownTime);
            console.log(`⏱️ Rule #${matchedRuleId} put on cooldown for ${cooldown} seconds.`);
        }
        ...
        if (finalReplies.replies && finalReplies.enableDelay && finalReplies.replyDelay > 0) {
            const firstReply = finalReplies.replies[0];
            const remainingReplies = finalReplies.replies.slice(1);
            if (remainingReplies.length > 0) {
                sendDelayedReplies(remainingReplies, finalReplies.replyDelay, sessionId, senderName, groupName, isGroup);
            }
            replyToReturn = [firstReply];
        } else {
            replyToReturn = finalReplies.replies || finalReplies;
        }
    }
