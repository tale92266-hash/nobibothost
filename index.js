...
// Initial Load
(async () => {
    // Corrected logic to ensure Node.js server starts first
    ...
    if (!fs.existsSync(ownersListFile)) {
        fs.writeFileSync(ownersListFile, JSON.stringify([], null, 2));
    }
    if (!fs.existsSync(ignoredOverrideUsersFile)) {
        fs.writeFileSync(ignoredOverrideUsersFile, JSON.stringify([], null, 2));
    }
    if (!fs.existsSync(specificOverrideUsersFile)) {
        fs.writeFileSync(specificOverrideUsersFile, JSON.stringify([], null, 2));
    }
    if (!fs.existsSync(settingsFilePath)) {
        fs.writeFileSync(settingsFilePath, JSON.stringify(settings, null, 2));
    }
