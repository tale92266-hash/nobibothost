...
// FIXED: pickNUniqueRandomly function
...
// COMPLETELY FIXED: resolveVariablesRecursively function
function resolveVariablesRecursively(text, maxIterations = 10) {
    let result = text;
    let iterationCount = 0;
    
    console.log(`🔧 Starting variable resolution for: "${result}"`);
    
    while (iterationCount < maxIterations) {
        let hasVariables = false;
        let previousResult = result;
        
        // STEP 1: Process Custom Random Variables FIRST (FIXED REGEX)
        const customRandomRegex = /%rndm_custom_(\d+)_([^%]+)%/g;
        result = result.replace(customRandomRegex, (fullMatch, countStr, tokensString) => {
            const count = parseInt(countStr, 10);
            console.log(`🎲 Processing custom random: count=${count}, tokens="${tokensString}"`);
            
            // Split tokens by comma and clean them
        ...
        // STEP 2: Process other random variables like %rndm_num_1_100%
        const otherRandomRegex = /%rndm_(\w+)_(\d+)(?:_(\d+))?%/g;
        result = result.replace(otherRandomRegex, (fullMatch, type, param1, param2) => {
            console.log(`🎲 Processing other random: type=${type}, param1=${param1}, param2=${param2}`);
            
            let varValue = '';
            if (type === 'num' && param2) {
                const min = parseInt(param1);
                const max = parseInt(param2);
            ...
            console.log(`✅ Other random result: "${varValue}"`);
            hasVariables = true;
            return varValue;
        });
        
        // STEP 3: Process static variables like %username%
        const staticVarRegex = /%(\w+)%/g;
        result = result.replace(staticVarRegex, (fullMatch, varName) => {
            const staticVar = VARIABLES.find(v => v.name === varName);
            if (staticVar) {
                console.log(`📝 Resolved static variable: %${varName}% = "${staticVar.value}"`);
                hasVariables = true;
                return staticVar.value;
            }
            return fullMatch; // Keep unchanged if variable not found
        });
        
        // Break if no changes made
        if (result === previousResult) {
            console.log(`🏁 No more variables to process. Final result: "${result}"`);
            break;
        }
        
        iterationCount++;
        console.log(`🔄 Iteration ${iterationCount}: "${result}"`);
    }
    
    if (iterationCount >= maxIterations) {
        console.warn(`⚠️ Maximum iterations (${maxIterations}) reached. Final result: "${result}"`);
    }
...
async function processMessage(msg, sessionId = "default") {
    ...
    // FIXED: Process reply with variables (ENSURE PROPER RESOLUTION)
    if (reply) {
        console.log(`🔧 Processing reply with variable resolution: "${reply}"`);
        
        // Convert newlines before processing variables
        reply = reply.replace(/\\n/g, '\n');
        
        // Resolve all variables
        reply = resolveVariablesRecursively(reply);
        
        console.log(`✅ Final processed reply: "${reply}"`);
    }
